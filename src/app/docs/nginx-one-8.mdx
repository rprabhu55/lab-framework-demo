...
1. Now click on **Run** for each of the NGINX Plus containers listed below. They will be added to the config sync group you just created.
...
1. Now let's add an NGINX OSS instance to our Config Sync group. SSH into your **NGINX OSS** component and install the NGINX Agent using the following command
    ```bash
...
1. Another approach is modify the Config Sync Group configuration such that is compatible with both NGINX Plus and NGINX Open Source instances.

...
The overall reachability and availability of your NGINX fleet is displayed here.

![Availability View](media/lab1-7.png)

    > Note: there is a default 24 hour period before NGINX One automatically removes *unavailable* NGINX instances.

**NGINX Versions**

We can see at a glance what proportion of our fleet is running Plus vs. OSS, and how many instances are running outdated or non-standard versions.

![NGINX Versions View](media/lab1-8.png)

**OS Distribution**

Similarly, we can see which OSes are part of the fleet.

![NGINX OS Distribution View](media/lab1-9.png)

**Certificates**

This chart provides insight on expiring certificates that need attention, as well as expired ones that should be removed.

![NGINX Certificates View](media/lab1-10.png)

**Configuration Recommendations**

NGINX One analyzes the configuration files on all of the instances it manages. When a configuration presents a potential security risk or is otherwise not in accordance with best practices, a recommendation is shown.

![NGINX Configuration Recommendations View](media/lab1-11.png)

**CVEs**

Any CVEs that apply to an instance in your fleet are shown here.

![NGINX CVEs View](media/lab1-12.png)

**CPU Utilization**

The instances with the highest CPU Utilization are shown here for quick visibility into potential performance issues.

![NGINX CPU Utilization View](media/lab1-13.png)

**Memory Utilization**

Top memory consumers are also shown here.

![NGINX Memory Utilization View](media/lab1-14.png)

**Disk Space Utilization**

Along with CPU and memory, instances with the highest disk utilization are shown here to surface possible disk space issues.

![NGINX Disk Space Utilization View](media/lab1-15.png)

**Unsuccessful Responses**

Instances with the most 4xx or 5xx errors appear here to identify potential attacks, misconfigurations, or upstream application issues.

![NGINX Unsuccessful Responses View](media/lab1-16.png)

**Network Usage**

Top network consumers can help identify targets for optimization. Unusual traffic patterns could potentially indicate an attack attempt.

![NGINX Network Usage View](media/lab1-17.png)
...
![NGINX Drilling Down View](media/lab1-18.png)
...
**Option 1:** When you add the first NGINX instance, the config from the NGINX instance will be used as the config for your Config Sync Group. We will be using this option for this portion of the lab.
...
1. The instructions on the screen tell you how to:
    ...
    The page shows the shell commands needed to accomplish this:
...
To remove the NGINX OSS instance from this instance group:
...
As you have seen so far, you can manage both NGINX Plus and OSS instances in the NGINX One console. In this portion of the lab, we will focus on NGINX OSS. We will see the benefit of from having consistent configuration across multiple NGINX OSS instances. When NGINX OSS instances are created that are pre-configured to use a Config Sync Group, they will automatically download their configuration on startup. This will save time, and ensure consistency between instances.

> TODO: Move the NGINX Docker OSS component here and update the component to pass the config sync group the user will create.
>
>   JW: Can you eyeball this section? I think this is what you meant when you wrote this. Not sure the correct terminology here is but I use **Docker** and **APIHeaderCheck**

Let's create a custom NGINX configuration before any NGINX instances are added to a Config Sync Group. The previous lab contained an existing NGINX configuration when the first NGINX instance was added. We will create a new Config Sync Group for this lab.

1. Create a new Config Sync Group, *<GetVariable name="petname" />-oss*.

1. After if it created, click on your new *<GetVariable name="petname" />-oss* group.
...
1. An editor window appears that allows you to define the configuration to use for the NGINX instances in this group.

    ![Manual Configuration](media/lab5-4.png)

    Use the following NGINX configuration here.
...
1. You can also add files by clicking *Add File* but this is limited to the **/etc/nginx/** path.

> TODO: Add screenshot

1. Create the file name **/etc/nginx/conf.d/default.conf** and use the following NGINX configuration.
...
1. A screenshot after you completed the above should look like below.

1. Select *Next*, then *Save and Publish*

1. Now let's add two NGINX Open Source instance in this Config Sync Group. First provide your Data Plane Key and 
   your Config Sync Group name below.
    ...
    Now start your NIGNX Open Source containers
    <Docker
        name="nginx-oss-1"
        desc="NGINX Open Source with Agent."
...
1. Finally, Use the check below to confirm the **/test_custom** URL is functional. This check the Header for **X-Test-Custom-App: true** with the URL **/test_custom**.
...
Great! Now you successfully created a custom NGINX configuration then pushed that to all NGINX instances that joined this Config Sync Group.

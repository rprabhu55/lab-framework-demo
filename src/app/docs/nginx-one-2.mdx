...
1. Create a custom NGINX configuration before any NGINX instances are added to a Config Sync Group.
...
1. Click the "Run" button in each of containers listed below. As a convenience, the dataplane key and Config Sync Group have already been provided to the containers.
...
1. Select the "Docker Container" tab.
    ...
    > TODO: Take a longer (in height) screenshot of the next image to show the missing command.

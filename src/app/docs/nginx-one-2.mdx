...
1. On the right, enter your unique lab deployment name, **<GetVariable name="petname" />**, then select *Create* on the bottom.
    ...
    ![Create New Config Sync Group](media/lab5-1-2.png)
...
![Details View](media/lab5-2-1.png)

On the *Configuration* tab, there is one view that defines the NGINX configurations to use for all NGINX instances in this Config Sync Group.

![Configuration View](media/lab5-2-2.png)
...
**Option 1:** Auto-generate the Config Sync Group's configuration by adding the first NGINX instance. The config from the NGINX instance will be used as
the config for your Config Sync Group.
...
We will now proceed with adding the first NGINX instance to the Config Sync Group. We will be using the configuration from this first NGINX instance as the
Config Sync Group's configuration.
...
But for this lab, we have simplified this process for you:
...
1. When you select the *Configuration* tab, notice the config here is the same as the first NGINX instance.
...
Now that we have two instances in the *<GetVariable name="petname" />* Config Sync Group, lets publish a new configuration to them.
...
We can now confirm the configuration changes on the NGINX instances by clicking the *Check* button for each NGINX instance below:
...
1. Select *Config Sync Groups* under *Manage* on the left to bring up a list of all Config Sync Groups.
    ...
    ![List Config Sync Group](media/lab5-19.png)

1. Delete your existing *<GetVariable name="petname" />* Config Sync Group

    ![Delete Config Sync Group](media/lab5-20.png)

1. Re-create the Config Sync Group, *<GetVariable name="petname" />*.

1. After if it is re-created, click on the *<GetVariable name="petname" />* group.
...
1. You can also add files by clicking *Add File* but this is limited to the **/etc/nginx/** path.
    ...
    ![Add File](media/lab5-21.png)

1. Create the file name **/etc/nginx/conf.d/default.conf**

    ![Name File](media/lab5-22.png)

1. Be sure the new file is selected in the text editor

    ![Select File](media/lab5-23.png)
...
1. A screenshot after you completed the above should look like below.
    ...
    ![Final Config View](media/lab5-24.png)

1. Select *Next*, then *Save and Publish*

    ![Final Diff](media/lab5-25.png)

1. Now let's add two NGINX Open Source instance in this Config Sync Group. Select *Run* for each NGINX OSS instances below.
    <Docker
        name="nginx-oss-1"
    ...
    There will be two NGINX OSS instances in your *<GetVariable name="petname" />* group.

    ![NGINX OSS Instances](media/lab5-26.png)

1. Finally, use the check below to confirm the **/test_custom** URL is functional. This check the Header for **X-Test-Custom-App: true** using the path **/test_custom**.
...
1. Repeat steps 1-6 for any remaining **<GetVariable name="petname" />** instance.

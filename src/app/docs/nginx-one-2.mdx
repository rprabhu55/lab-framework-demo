...
The NGINX One Console displays the number of certifiates currently installed on the NGINX instance as well as the current status of the certificates, expiration date,
and the certificates Subject Name.

We'll get to explore this in greater detail later in the lab when we install a SSL/TLS certificate.

#### Instance CVEs

The NGINX One Console displays the number of CVEs associated with the NGINX instance as well as the CVEs severity.  You can also find out more information about each CVE
...
The NGINX One Console also provides various details about the NGINX instance such at:
...
You now see the Dashboard view like the example shown below.
...
Note there is a default 24 hour period before NGINX One deletes *unavailable* NGINX Instances are automatically cleaned.
...
This chart provides insight on expiring certificates that need attention, as well as expired ones that should be pruned.
...
Instances with the most 4xx or 5xx errors appear here to identify potential attacks or misconfigurations.
...
Now lets take a look at what the **/etc/nginx/nginx.conf** configuration is doing:
...
1. Notice the editor has a Best Practice notice indicating that *default_server* should set.  Update the configuration to acknowledge this recommendation.

The entire **/etc/nginx/conf.d/default.conf** file should now look as follows, commented out configurations have been removed:
...
To check that the <GetVariable name="nginx-plus-1" /> instance is now listening on 443 click the *Check* button below.

...
Lets go ahead and try to create a custom NGINX configuration before any NGINX instances get added on a Config Sync Group.
...
1. Use the following NGINX configuration for **/etc/nginx/nginx.conf**
    ```nginx
    user  nginx;
    worker_processes  auto;
...
1. Now lets add the first NGINX instance in this Config Sync Group.
...
>       [JW]: Removing the lab portion and leaving the explanation..

Note: You will be able to mix NGINX Plus and NGINX Open Source instances with the exception that the configuration must be compatible across NGINX OSS and NGINX Plus.
...
Great! Now you completed option 2 which pushes a customized NGINX configuration for all NGINX instances that join a Config Sync Group.

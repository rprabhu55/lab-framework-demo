...
As you have seen so far, you can manage both NGINX Plus and OSS instances in the NGINX One console. In this portion of the lab, we will focus on NGINX OSS. We will see the benefit of from having consistent configuration across multiple NGINX OSS instances. When NGINX OSS instances are created that are pre-configured to use a Config Sync Group, they will automatically download their configuration on startup. This will save time, and ensure consistency between instances.
    ...
    > Note: It is possible to mix NGINX Plus and NGINX OSS instances in a Config Sync Group with the condition that the configuration used must be compatible across the NGINX Plus and NGINX OSS instances. For example, you will not be able to publish configurations that are exclusive to NGINX Plus, like [JWT authentication](https://docs.nginx.com/nginx/admin-guide/security-controls/configuring-jwt-authentication/) to NGINX OSS.

Since NGINX Plus has features and capabilities beyond the OSS edition, it is a good practice to create separate Config Sync Groups for them. Let's do that now.
...
Let's create a custom NGINX configuration before any NGINX instances are added to a Config Sync Group.

1. The previous lab will contain an existing NGINX configuration when the first NGINX instance was added. We will create a new Config Sync Group for this lab.
...
1. Use the following NGINX configuration for the **/etc/nginx/nginx.conf** file:
...
1. Create another file, **/etc/nginx/conf.d/default.conf**, and use the following NGINX configuration.
...
1. Now let's add the first NGINX instance in this Config Sync Group.

> TODO: Use button here?

1. Now you can use the check below to confirm the **/test_custom** URL is functional.

> TODO: Add ResponseCheck button

Great! Now you have completed option 2 (mentioned earlier in the lab), which pushes a customized NGINX configuration for all NGINX instances that join a Config Sync Group.

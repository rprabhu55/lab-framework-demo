...
Now that we added our first NGINX Plus instance, let's add our second NGINX Plus instance, <GetVariable name="nginx-plus-2" />, to the Config Sync Group.
...
### Removing or Changing NGINX Instances in Config Sync Group

We will now go though the steps to remove an instance from a config sync group. In the example here, the main configuration from our config sync group is not compatible with NGINX Open Source and we are going to remove it.

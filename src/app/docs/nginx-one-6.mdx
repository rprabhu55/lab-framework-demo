...
1. Make sure the edits you made are valid, then save and publish these changes. Now observe the agent.log feed on both instances as the configuration update is applied. For both instances you will see a series of log messages appear, indicating that the configuration change is being sent and applied to each instance.
    ![](media/lab6-4.png)

...
As we are using a fresh environment for this lab, **we will only use the first method** to add our NGINX instances to the config sync group.
...
1. SSH into your **NGINX Plus** component and use the following command to install and run the NGINX Agent. Make sure to use your dataplane key where indicated.
...
1. The following code is added to the bottom of the **agent-dynamic.conf** file and save the config file.
...
This NGINX Instance will then appear as an instance managed by a config sync group.
...
1. Open an SSH window for the **Lab Framework** VM and locate the running NGINX Plus Docker container using the following command:
    ...
    ```
    This will open up a bash shell session in the container. Once inside, use the same command in step 1 to access the NGINX Agent log within the container.
    ```bash
    sudo tail -f /var/log/nginx-agent/agent.log
    ```
...
1. Navigate to your sync group's configuration tab. Here, you can make any valid edits to the Config Sync Group configuration that you wish. In this case, we will add the following section to the server block of the /etc/nginx/conf.d/default.conf file:
    ...
    ```
1. Save and publish these changes. Now pay attention to the agent.log feed on both instances as the configuration update is applied.
---
### De-synchronizing a managed instance

What happens if we decide to make changes to the config of a particular NGINX instance which is under the management a Config Sync Group?

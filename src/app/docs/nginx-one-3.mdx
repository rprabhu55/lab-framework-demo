...
1. Because the hostname is used as the name of the instance in NGINX One, you should change the hostname to something that identifies it as yours, such as **<GetVariable name="petname" />-ubuntu-plus**.
...
Config Sync Group you created above. In the command below, confirm the following substitutions:

    - Confirm **DATA_PLANE_KEY** is set using the Data Plane Key you created above.
    ...
    ```bash
    curl https://agent.connect.nginx.com/nginx-agent/install | DATA_PLANE_KEY='{{NGINX_AGENT_SERVER_TOKEN}}' sh -s -- -y -c {{petname}}-ubuntu-plus
    ```
...
1. Execute the *docker run* command outlined in Step 6 to start the NGINX Plus with Agent container. Use the shell
command below and confirm the following substitutions:

    - Confirm **NMS_SERVER_TOKEN** is using the Data Plane Key you created above.
    ...
    --env=NGINX_AGENT_INSTANCE_GROUP={{petname}}-ubuntu-plus \
...
1. Remove the NGINX Plus container using command below with the following substitution:
    ...
    *Instances* list of the NGINX One Console.
      - Hint: You saw it in Step 10 of the previous section.

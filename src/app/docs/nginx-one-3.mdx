...
Ensure that you are working on the NGINX Plus instance (default hostname ip-10.1.1.7), and run the following command below using only lowercase characters and hyphens.
...
1. From the NGINX Plus instance, run the following command to install the NGINX Agent, and automatically join the Config Sync Group you created above. In the command below, make the following substitutions:
...
1. Execute the *docker run* command outlined in Step 6 to start the NGINX Plus with Agent container. Use the shell command below, with the following substitutions:
    ...
    - Substitute **config-sync-group** with the name Config Sync Group you just created
    - Includes mapping host's port 8080 to container's port 80
...
1. In the UDF deployment for this lab, click *Details* for the **NGINX Plus** component.
    ...
    ![Select the NGINX component details](media/cleanup-10.png)
